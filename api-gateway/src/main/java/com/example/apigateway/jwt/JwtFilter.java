...
public class JwtFilter implements WebFilter, ApplicationContextAware {
    ...
    private final String[] FREE_PATHS = {
            ...
            "/user/signup",
            "/user/vaild",
            "/api/news/**",        // /api/news/stock, /api/news/crypto를 위해 수정
            "/chat/all/**",        // /chat/all을 위해 수정
            "/ws/**",             // WebSocket 연결을 위한 경로 추가
            "/ws/info",           // STOMP 연결 정보
            "/ws/connect",        // STOMP 연결
            // "/api/assets"
            "/alert/test/trigger"
    ...
    public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
        // 필요시 사용 (단, 레디스로 사용할것이라 미비할듯, 없을듯 => 예상)
...
//        }
//        return null;
        // 토큰이 없으면 그대로 요청을 전달 -> 게이트웨이에서 컷 401 로 처리
        return chain.filter(exchange);
    }
}
